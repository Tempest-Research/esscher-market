...
def _open_windows_pin_file(kernel32, output_dir: Path) -> int:
    """Create an unreplaceable empty child that keeps the validated tree in place."""

    path = output_dir / f".esscher-capture-{uuid.uuid4().hex}.pin"
    handle = kernel32.CreateFileW(
        str(path),
        _WINDOWS_FILE_READ_ATTRIBUTES | _WINDOWS_DELETE,
        _WINDOWS_FILE_SHARE_READ | _WINDOWS_FILE_SHARE_WRITE,
...
def _pin_windows_output_directory(output_dir: Path):
    ...
    While it is held, create a delete-on-close child pin; that child keeps the
    directory and its ancestors in place after the directory handle is released.
    """
