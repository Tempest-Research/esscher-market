...
- Because the bound `main` baseline has no configured type checker and does not pass an unsuppressed whole-repository type check, the RC uses this approved production no-new-regressions type policy:
  ...
  - the regression checker and CI entry point themselves must have zero Pyright `error` diagnostics;
  - `tests/` and other `scripts/` remain covered by lint, compilation, and the full runtime test suite, while their historical typing debt is tracked separately from this RC gate;
  - do not add ignores, suppressions, generated waiver baselines, or relaxed Pyright configuration merely to pass this gate;
  - record the exact tool version, commands, base/RC identities, JSON reports, changed production-file set, normalization method, counts, and comparison result in the integration evidence.
