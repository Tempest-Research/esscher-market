...
- Earnings BMO/AMC and macro clocks are carried per-cohort and cannot be
  blended.
- The verified exit plan must bind the exact opening permit event and policy;
  a valid clock set for another event or policy is rejected as
  `EXIT_PLAN_UNVERIFIED` before intent persistence or mutation.
...
- A normal close cannot run before that frozen time-exit clock. Both opening
  and closing permits must be active at the lifecycle clock, and the worker
  only accepts a named `PAPER` account with atomic `MULTI_LEG` requests.
...
Every side-effect intent is persisted before the next mutation as canonical
request JSON plus its SHA-256 digest. The durable intent carries the open
permit, client order ID, full legs, expected quantity, price, account/order
class, correlation, policy, snapshot, reservation, and event binding. SQLite
permits one `OPEN` and one `CLOSE` intent per event, so a restart or alternate
close permit cannot re-submit a broker mutation.

Broker acknowledgement is never fill proof: the worker always reads fresh
account, order, and position truth and checks it against the full persisted
request before it persists a fill or flat result. Unknown, partial, incident,
or exposure-bearing states stop new entries while preserving reconciliation and
close authority.

## Deterministic behaviors

- **Duplicate ticks:** a permit can be submitted once; an alternate close
  permit for the same event is also rejected with `DUPLICATE_TICK` rather than
  creating a second close order.
- **Restart recovery:** `recover_open_state` and `recover_flatness` resume from
  the canonical durable request plus fresh broker/ledger truth rather than
  replaying intent. Without a submitted close intent, flatness is
  `MANUAL_REQUIRED`, never a fabricated terminal success.
...
spans every transition, timeout, restart point, durable alternate-close
rejection, full request/passport binding, permit and exit-plan tampering,
PAPER-boundary rejection, partial fill, broker outage, stale/future-dated
truth, clock jump past flattening, non-flat close, and the mutation gate.
